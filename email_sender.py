...
def get_scylla_build_info():
    for build_info in Path(os.getenv("WORKSPACE", ".")).glob("**/00-Build.txt"):
        return dict([l.split(': ', maxsplit=1) for l in build_info.read_text().splitlines()]) or {}
...
def create_report(results, scylla_version, **kwargs):
    build_info = get_scylla_build_info() or {}
    scylla_version = f"{build_info.get('scylla-version', scylla_version)}-{build_info.get('scylla-release', 'unknown')}"
